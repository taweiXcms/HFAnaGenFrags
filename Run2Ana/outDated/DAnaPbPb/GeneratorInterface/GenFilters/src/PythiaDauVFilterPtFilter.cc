#include "GeneratorInterface/GenFilters/interface/PythiaDauVFilterPtFilter.h"
...
PythiaDauVFilterPtFilter::PythiaDauVFilterPtFilter(const edm::ParameterSet& iConfig) :
  ...
  motherPt(iConfig.getUntrackedParameter("MotherPt", 0)),
  motherMaxEta(iConfig.getUntrackedParameter("MotherMaxEta", 9999.)),
  motherMinEta(iConfig.getUntrackedParameter("MotherMinEta", -9999)),
  motherStatus(iConfig.getUntrackedParameter("MotherStatus", 0)),
  ...
  particlePt(iConfig.getUntrackedParameter("ParticlePt", 0.)),
  particleMaxEta(iConfig.getUntrackedParameter("ParticleMaxEta", 9999.)),
  particleMinEta(iConfig.getUntrackedParameter("ParticleMinEta", -9999.)),
  particleStatus(iConfig.getUntrackedParameter("ParticleStatus", 0))
{
  ...
  cout << "----------------------------------------------------------------------" << endl;
  cout << "--- PythiaDauVFilterPtFilter" << endl;
...
PythiaDauVFilterPtFilter::~PythiaDauVFilterPtFilter()
{
...
bool PythiaDauVFilterPtFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  bool accepted = false;
  ...
  for (HepMC::GenEvent::particle_iterator p = myGenEvent->particles_begin(); p != myGenEvent->particles_end(); ++p) {
    ...
    if ((*p)->momentum().perp() < particlePt) continue ;
    if ((*p)->momentum().eta() > particleMaxEta || (*p)->momentum().eta() < particleMinEta) continue ;
    if (particleStatus != 0 && (*p)->status() != particleStatus) continue;
    ...
    if (abs(motherID) == abs((*des)->pdg_id()) && (*des)->momentum().perp() > motherPt && (*des)->momentum().eta() < motherMaxEta && (*des)->momentum().eta() > motherMinEta && ((*des)->status() == 0 || (*des)->status() == motherStatus)) {
	  OK = 1; 
	  break;
	}
      }
...
DEFINE_FWK_MODULE(PythiaDauVFilterPtFilter);
